...
class Imagevalidator extends Validator implements ValidatorContract
{
	...
	protected function checkDimension($rule, $dimension=0)
	{
		...
		if ($rule == '*')
		{
			$message = $this->translator->trans('imagevalidator::validation.anysize');
			$pass = true;
		}
		else if ( preg_match('/^(\d+)\-(\d+)$/', $rule, $matches) )
		{
			...
			$message = $this->translator->trans('imagevalidator::validation.between', compact('size1','size2'));
		...
		else if ( preg_match('/^([<=>]*)(\d+)$/', $rule, $matches) )
		{
			...
			{
				case '>':
					$message = $this->translator->trans('imagevalidator::validation.greaterthan', compact('size'));
					$pass = $dimension > $size;
					break;
				case '>=':
					$message = $this->translator->trans('imagevalidator::validation.greaterthanorequal', compact('size'));
					$pass = $dimension >= $size;
					break;
				case '<':
					$message = $this->translator->trans('imagevalidator::validation.lessthan', compact('size'));
					$pass = $dimension < $size;
					break;
				case '<=':
					$message = $this->translator->trans('imagevalidator::validation.lessthanorequal', compact('size'));
					$pass = $dimension <= $size;
					break;
				case '=':
				case '':
					$message = $this->translator->trans('imagevalidator::validation.equal', compact('size'));
					$pass = $dimension == $size;
					break;
				default:
